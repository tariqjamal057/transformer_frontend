...
const sampleHeaders = [
  {
    ...
    discomReceiptNoteDate: "2024-01-20",
  },
];

const GPReceiptNoteBulkUploadModal = ({ open, handleClose }) => {
  ...
  const bulkUploadMutation = useMutation({
    mutationFn: (formData) =>
      api.post("/gp-receipt-notes/bulk-upload", formData, {
        headers: {
    ...
    onSuccess: (response) => {
      setAlertBox({
        open: true,
        msg: `Bulk upload successful! Created ${response.data.createdNotes.length} notes.`,
        error: false,
      });
  ...
  const handleDownloadSample = () => {
    ...
    saveAs(data, "sample_gp_receipt_note.xlsx");
  };
  ...
  return (
    <Dialog open={open} onClose={handleModalClose} fullWidth>
      ...
      <DialogActions>
        ...
        >
          {bulkUploadMutation.isLoading ? "Uploading..." : "Upload Notes"}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
export default GPReceiptNoteBulkUploadModal;
